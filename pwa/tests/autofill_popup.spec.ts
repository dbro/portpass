...
// ---------------------------------------------------------------------------
// Relay mode: popup bridges between bookmarklet and an already-open main tab
// ---------------------------------------------------------------------------

test.describe('Autofill relay mode', () => {
  ...
  // Opens a main Portpass tab (non-popup), unlocks a new vault, and returns the page.
  async function openMainTab(context: BrowserContext): Promise<Page> {
    const main = await context.newPage()
    await main.goto(PORTPASS_URL)
    await main.getByRole('button', { name: 'Create one' }).click()
    await main.getByPlaceholder('Master password').fill('testpassword')
    await main.getByRole('button', { name: 'Create vault' }).click()
    await expect(main.getByPlaceholder('Search vault')).toBeVisible({ timeout: 10000 })
    return main
  }

  test('relay popup does not show unlock UI when main tab is unlocked', async ({ context }) => {
    await openMainTab(context)
    const { popup } = await openPortpassPopup(context)
    // 300 ms relay ping + buffer; main tab responds instantly so relay succeeds quickly
    await popup.waitForTimeout(500)
    await expect(popup.getByRole('button', { name: 'Open vault file' })).toHaveCount(0)
    await expect(popup.getByRole('button', { name: 'Create one' })).toHaveCount(0)
  })

  test('relay delivers correct credentials from the main tab', async ({ context }) => {
    const main = await openMainTab(context)
    await main.getByRole('button', { name: 'New', exact: true }).click()
    await main.getByPlaceholder('e.g. Bank of America').fill('Relay Test')
    await main.locator('input.mono').first().fill('hunter2')
    await main.locator('input.input').nth(2).fill('alice')
    await main.locator('.autotype-input').fill('\\u\\t\\p\\n')
    await main.getByRole('button', { name: 'Save' }).click()
    await main.locator('.record-row', { hasText: 'Relay Test' }).click()

    const { opener, popup } = await openPortpassPopup(context)
    await popup.waitForTimeout(500)  // Wait for relay mode to activate

    await doKeyExchange(opener)
    const response = await sendQuery(opener)

    expect(response.type).toBe('record')
    expect(response.title).toBe('Relay Test')
    expect(response.autotype).toBe('\\u\\t\\p\\n')
    expect(response.fields.u).toBe('alice')
    expect(response.fields.p).toBe('hunter2')
  })

  test('relay returns error when main tab has no record selected', async ({ context }) => {
    await openMainTab(context)  // Unlocked, no record selected
    const { opener, popup } = await openPortpassPopup(context)
    await popup.waitForTimeout(500)
  ...
  test('relay popup falls back to unlock UI when no main tab is open', async ({ context }) => {
    const { popup } = await openPortpassPopup(context)
    // Relay ping times out (300 ms), then WASM loads and unlock UI appears
    await popup.waitForSelector('button:text("Create one")', { timeout: 10000 })
  })
